...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(char => char).map(entry => entry match { case (x, y) => (x, y.length) }).toList.sorted
  ...
  {
    val superMap = x.toMap
    y.foldLeft(superMap)((resultMap, yOcc) => {
      val xOccInt = resultMap(yOcc._1)
      val newOccInt = xOccInt -yOcc._2
      if (newOccInt <= 0) resultMap - yOcc._1
      else resultMap.updated(yOcc._1, newOccInt)
    }).toList.sorted
  }
